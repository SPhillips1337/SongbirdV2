...
- **Multi-Agent Orchestration**: Specialized agents for Artist, Music, and Lyrics generation
- **Suggestion Engine**: AI-powered song ideas based on your generation history
- **Trending Intelligence**: Real-time trend injection via Perplexity API
- **Centralized Band Manager**: Persistent virtual bands with discography tracking
- **Poetry Mode**: Elevated lyricism for high-art songwriting
- **Album Mode**: Generate cohesive multi-track albums with consistent themes
- **Smart Caching**: Reduced API costs through intelligent response caching
...
3. **Run the Workflow**:
   ...
   # Advanced Features
   python app.py --suggest  # Get AI-powered song suggestions
   python app.py --trending --genre DUBSTEP  # Inject real-world trends
   python app.py --poetic --direction "about lost memories"  # Poetry mode
   python app.py --band "The Neon Ghosts" --album --theme "Cyberpunk rebellion"  # Create a band
   ```
